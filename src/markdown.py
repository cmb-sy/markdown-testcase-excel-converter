...
class MarkdownTestParser:
    ...
    def __init__(self, markdown_content: str, config: Config):
        ...
        self.pattern_section = re.compile(
            self.config.columns.section.md_pattern, re.MULTILINE
        )
        self.pattern_subsection = re.compile(
            self.config.columns.subsection.md_pattern, re.MULTILINE
        )
        self.pattern_testcase = re.compile(
            self.config.columns.testcase.md_pattern, re.MULTILINE
        )
        self.pattern_step = re.compile(
            self.config.columns.step.md_pattern, re.MULTILINE
        )
        self.pattern_expectation = re.compile(
            self.config.columns.expectation.md_pattern, re.MULTILINE
        )
        ...
        self.subsection_map = (
            {}
        )  # '大分類名:中分類名'をキーとし、その番号を値とするディクショナリ
    ...
    def parse(self) -> pd.DataFrame:
        ...
        for i, line in enumerate(lines):
            ...
            if section_match:
                ...
                current_subsection = (
                    None  # Reset subsection when a new section is found
                )
                ...
                if (
                    current_subsection != last_subsection
                    or current_section != last_section
                ):
                ...
                subsection_num = (
                    self.subsection_map.get(
                        f"{current_section}:{current_subsection}", 0
                    )
                    if current_subsection
                    else 0
                )
                hierarchical_no = (
                    f"{section_num}-{subsection_num}-{self.testcase_count}"
                )
                ...
                if expectations:
                    # 試験内容を準備（1回だけ）
                    test_content = "\n".join(
                        [f"{j + 1}. {step}" for j, step in enumerate(steps)]
                    )

                    # 各確認事項ごとに別々の行を作成
                    for i, expectation in enumerate(expectations):
                        # 確認事項の番号を追加（同じテストケース内での連番）
                        expectation_no = f"{hierarchical_no}-{i+1}"

                        # データフレーム用の行データ作成（新しい順序）
                        self.data.append(
                            [
                                expectation_no,  # 階層化されたNO（確認事項番号付き）
                                current_section,  # 大分類
                                current_subsection,  # 中分類
                                test_case_name,  # 小分類
                                (
                                    test_content if i == 0 else ""
                                ),  # 試験内容（最初の行のみ）
                                expectation,  # 確認事項（1項目のみ）
                            ]
                        )
                else:
                    # 確認事項がない場合は元の処理
