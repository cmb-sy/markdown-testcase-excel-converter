...
def estimate_column_width(
    text, font_name="Meiryo UI", font_size=11, min_width=8, max_width=100
):
    """
...
def estimate_row_height(
    row_data, font_name="Meiryo UI", font_size=11, min_height=15, line_height_factor=1.5
):
    """
...
class ExcelWriter:
    ...
    def __write_test_specification_sheet(
        self,
        workbook,
        ...
        if merge_cells:
            multi_idx_cols = [
                v["name"]
                for k, v in self.config.columns.model_dump().items()
                if v.get("multi_idx")
            ]
            if multi_idx_cols:
        ...
        worksheet = (
            workbook[sheet_name]
            if template_used and sheet_name in workbook.sheetnames
        ...
        if template_used:
            ...
            for i, row in enumerate(df_excel.itertuples(index=False)):
                ...
                if auto_adjust_height:
                    row_height = estimate_row_height(
                        row, self.config.excel_settings.font_name
                    )
                ...
                for j, value in enumerate(row):
                    ...
                    apply_cell_style(
                        ...
                        alignment=Alignment(
                            horizontal=self.config.columns.model_dump()[
                                list(self.config.columns.model_dump().keys())[j]
                            ]["horizontal"],
                            vertical=self.config.columns.model_dump()[
                                list(self.config.columns.model_dump().keys())[j]
                            ]["vertical"],
                            wrap_text=True,
                        ),
                ...
                if preserve_additional_columns:
                    ...
                    if row_id in additional_columns_data:
                        for col_idx, cell_value in additional_columns_data[
                            row_id
                        ].items():
                            col_letter = get_column_letter(col_idx + 1)
                            cell = worksheet[f"{col_letter}{last_row + i}"]
            ...
            for j, col_name in enumerate(self.columns):
                ...
                apply_cell_style(
                    cell,
                    font=Font(
                        name=self.config.excel_settings.font_name,
                        bold=True,
                        color="ffffff",
                    ),
                ...
                worksheet.column_dimensions[
                    col_letter
                ].width = self.config.columns.model_dump()[
                    list(self.config.columns.model_dump().keys())[j]
                ][
                    "length"
                ]
        ...
        if auto_adjust_width:
            ...
            for j, col_name in enumerate(self.columns):
                col_letter = get_column_letter(j + 1)  # 1-indexed
                col_config = self.config.columns.model_dump()[
                    list(self.config.columns.model_dump().keys())[j]
                ]

                # この列の最大幅を計算（ヘッダーも含む）
                max_width = estimate_column_width(
                    col_name, self.config.excel_settings.font_name
                )
                ...
                for row in range(start_row, end_row + 1):
                    ...
                    if cell_value:
                        estimated_width = estimate_column_width(
                            cell_value, self.config.excel_settings.font_name
                        )
            ...
            for col_idx in range(7, 14):  # G列(7)からM列(13)まで
                ...
                max_width = estimate_column_width(
                    header_cell.value, self.config.excel_settings.font_name
                )
                    ...
                    if cell_value != current_value or i == row_count - 1:
                        ...
                        if current_value is not None and row - start_row > 1:
                            ...
                            worksheet.merge_cells(
                                f"{col_letter}{start_row}:{col_letter}{end_row-1}"
                            )
    ...
    def __call__(
        ...
        try:
            ...
            if template_path and template_path.exists():
                ...
                # テンプレートのシートにデータを書き込む
                self.__write_test_specification_sheet(
                    workbook,
                    merge_cells,
                    template_used=True,
            ...
            raise PermissionError(
                f"出力先のファイルを開いている可能性があります。エクセルファイルを閉じてください。"
            )
        except Exception as e:
            raise ValueError(
                f"エクセルファイル出力中に不明なエラーが発生しました：\n{e}"
            )
